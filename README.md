...
- A C compiler, to compile the bundled Tree-sitter parser once at install time:
    ...
    - **Windows**: run from a "Developer Command Prompt for VS" after installing [Visual Studio Build Tools](https://visualstudio.microsoft.com/visual-cpp-build-tools/) so MSVC `cl.exe` + `link.exe` are on `PATH`. MinGW `gcc` also works if available. Standalone LLVM Clang alone is *not* sufficient — it relies on `link.exe` from MSVC.
